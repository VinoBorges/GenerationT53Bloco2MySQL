...
SELECT * FROM tb_produtos WHERE NOT categoria_id = 2; -- negação

SELECT * FROM tb_produtos WHERE NOT categoria_id = 2 ORDER BY nome;

SELECT * FROM tb_produtos WHERE preco IN (5,10,15); /*semelhante a OR, só que engloba todos valores de uma vez*/ 

SELECT * FROM tb_produtos WHERE preco BETWEEN 5 AND 15; /*procurar dentro de um intervalo de valores, pode ser feito com datas também*/

SELECT * FROM tb_produtos WHERE dtvalidade BETWEEN '2022-03-10' AND '2022-03-15' ORDER BY dtvalidade DESC;

SELECT * FROM tb_produtos WHERE nome LIKE "%ra%";

SELECT * FROM tb_produtos WHERE nome LIKE "%ra"; /* apenas palavras terminadas em "ra"*/

SELECT * FROM tb_produtos WHERE nome LIKE "ra%"; /*apenas palavras iniciadas em "ra"*/

SELECT count(*) FROM tb_produtos; /*conta a quantidade*/
...
SELECT categoria_id,  avg(preco) as Média FROM tb_produtos GROUP BY categoria_id; /*group by, agrupa por categoria*/

 SELECT max(preco) FROM tb_produtos;
 
 SELECT min(preco) FROM tb_produtos;
 
 SELECT * FROM tb_produtos INNER JOIN tb_categorias ON tb_produtos.categoria_id = tb_categorias.id; /*a chave estrangeira tem que ser igual a chave primaria da tb_categoias*/ 
